...
use ndarray::ArrayD;
use nifti::{NiftiObject, ReaderOptions, IntoNdArray, NiftiHeader};
use std::fs::File;
use std::io::{BufRead, BufReader};
use std::path::Path;

pub fn load_timestamps(path: &Path) -> Result<Vec<f64>> {
    let file = File::open(path).with_context(|| format!("Failed to open timestamps file: {:?}", path))?;
    ...
    for line in reader.lines() {
        ...
        for part in line.split_whitespace() {
            let t: f64 = part.parse().with_context(|| format!("Failed to parse timestamp: {}", part))?;
            timestamps.push(t);
        }
    }
...
pub fn load_nifti(path: &Path) -> Result<NiftiData> {
    let obj = ReaderOptions::new().read_file(path).with_context(|| format!("Failed to read NIfTI file: {:?}", path))?;
    let header = obj.header().clone();
    let volume = obj.into_volume().into_ndarray::<f32>().with_context(|| "Failed to convert volume to ndarray")?;
